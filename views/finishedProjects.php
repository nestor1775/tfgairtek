...
<h1 class="m-4 text-lg flex justify-center">Proyectos finalizados</h1>
...
<ul>
    <?php
    foreach ($proyectosInactivos as $inactivo) {
        echo "<li class='m-4'>
                    <a href='index.php?action=viewProject&id=" . $inactivo['id'] . "'
                    class='block bg-white shadow-lg p-4 rounded-lg hover:bg-gray-100 cursor-pointer h-full w-full'>
                        <span class='text-gray-800 font-semibold'>
                            " . htmlspecialchars($inactivo['nombre']) . "
                        </span>
                    </a>
                </li>
";
